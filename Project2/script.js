...
var contractAddress = '0x73007CC145b9F509eC1e30B503c0E0de2aeC7d40'; // FIXME: fill this in with your contract's address/hash
...
async function getDebtors() 
{
	...
	{
		usersSet.add(listIOUs[i].debtor.toLowerCase());
	}
	return Array.from(usersSet);
}

async function getCreditorsOf(user) 
{
	var usersSet = new Set();
	const listIOUs = await BlockchainSplitwise.methods.getIOUsArray().call();
	for(var i=0; i<listIOUs.length; i++)
	{
		if(listIOUs[i].debtor.toLowerCase() == user.toLowerCase())
		{
			usersSet.add(listIOUs[i].creditor.toLowerCase());
		}
	}
	return Array.from(usersSet);
}
...
async function getTotalOwed(user) 
{
	...
	for(var i=0; i<listIOUs.length; i++)
	{
		if(listIOUs[i].debtor.toLowerCase() == user.toLowerCase())
		{
			totalOwed += parseInt(listIOUs[i].amount);
		}
	}
	return totalOwed;
}
...
async function getLastActive(user) 
{
	...
	for(var i=0; i<listIOUs.length; i++)
	{
		if(listIOUs[i].debtor.toLowerCase() == user.toLowerCase() || listIOUs[i].creditor.toLowerCase() == user.toLowerCase())
		{
			if(listIOUs[i].date > lastIOUDate)
			{
				lastIOUDate = listIOUs[i].date;
			}
		}
	}
	
	if(lastIOUDate == 0)return null;
	return lastIOUDate;
}
...
async function add_IOU(creditor, amount) 
{
	...
	await BlockchainSplitwise.methods.add_IOU_with_date(creditor, amount, date).send({from:web3.eth.defaultAccount, gas : 1000000});

	var cyclePath = await doBFS(creditor.toLowerCase(), web3.eth.defaultAccount.toLowerCase(), getCreditorsOf);
	//TODO: remove cycle
	if(cyclePath){
		cyclePath.push(creditor.toLowerCase());
	    await BlockchainSplitwise.methods.delete_cycle(cyclePath).send({from:web3.eth.defaultAccount, gas : 1000000});
	}
}
